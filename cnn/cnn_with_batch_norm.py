...
class Model:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            batch_norm1 = tf.layers.batch_normalization(inputs=X_img, training=self.training)
            conv1 = tf.layers.conv2d(inputs=batch_norm1, filters=32, kernel_size=[3, 3], padding="SAME",
                                     activation=tf.nn.relu)
            pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], padding="SAME", strides=2)
            # dropout1 = tf.layers.dropout(inputs=pool1, rate = 0.7, training=self.training)

            batch_norm2 = tf.layers.batch_normalization(inputs=pool1, training=self.training)
            conv2 = tf.layers.conv2d(inputs=batch_norm2, filters=64, kernel_size=[3, 3], padding="SAME",
                                     activation=tf.nn.relu)
            pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2, 2], padding="SAME", strides=2)
            # dropout2 = tf.layers.dropout(inputs=pool2, rate=0.7, training=self.training)

            batch_norm3 = tf.layers.batch_normalization(inputs=pool2, training=self.training)
            ...
            batch_norm4 = tf.layers.batch_normalization(inputs=flat, training=self.training)
            dense4 = tf.layers.dense(inputs=batch_norm4, units=625, activation=tf.nn.relu)
